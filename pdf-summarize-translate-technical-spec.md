...
| **一鍵部署** | Docker Compose 一鍵起服務，模型權重**烤進 image**、部署即帶模型（見 §8） |
...
2. **summarize-service**：**Ollama + Qwen3.5-4B（Q4_K_M GGUF）已烤進 image**，對外暴露 OpenAI 相容 `/v1/chat/completions`（gateway 以標準 client 呼叫，不綁定廠商 SDK；`<think>` 推理輸出會被過濾）。
...
> 選型原則：**開放權重、可離線、中文能力優先、8GB RAM 可跑**。定案模型 **Qwen3.5-4B**（2026 最新世代，支援 201 語言、中文最強）；翻譯採**雙軌**——技術文用 Qwen3.5 一模兩用（術語一致），通用/多語才走 **NLLB-200**（CTranslate2 優化、CPU 上快）。雙用決策依 §3.3 Phase 1 實測（該階段以 qwen2.5:3b 驗證，正式版升級為 qwen3.5:4b）。

### 3.1 預設（CPU 主機）

| 用途 | 模型 | 量化 | 記憶體 | 備註 |
|---|---|---|---|---|
| 摘要 + 技術文翻譯 | **Qwen3.5-4B** | Q4_K_M GGUF | ~2.6GB | 一模兩用；Qwen3.5 支援 201 語言、中文最強、術語一致 |
| 通用/多語翻譯（可選） | NLLB-200-distilled-600M | int8 | ~0.6GB | 200+ 語言、速度快；密集技術文品質較弱 |

**總預算**：預設只需 Qwen3.5-4B（~2.6GB 權重，含 KV 約 4GB）；加 gateway 與系統開銷 ≈ **~6GB，8GB RAM 可跑**（低並發）。若另開 NLLB 再 +0.6GB。
...
| 環境 | 摘要模型 | 說明 |
|---|---|---|
| CPU / 8GB | Qwen3.5-4B Q4_K_M | 預設（本專案採用） |
| GPU 8–12GB | Qwen3.5-8B Q4_K_M | 品質明顯提升 |
| GPU 16GB+ | Qwen3.5-14B / 30B-A3B | 長文摘要更完整 |
...
## 8. Docker Compose 部署（自帶模型）

**定案：權重烤進 image**，部署即帶模型、開機零下載、跨平台可上雲。三個服務：

```yaml
services:
  summarize-service:              # Ollama + Qwen3.5-4B（Q4_K_M）已烤進 image
    build:
      context: ./summarize-service
      args: { MODEL: qwen3.5:4b }
    expose: ["11434"]

  gateway:                        # 上傳/擷取/摘要/翻譯/文字雲/報告/快取
    build: { context: ., dockerfile: backend/Dockerfile }
    environment:
      SUMMARIZE_URL: http://summarize-service:11434/v1
      SUMMARIZE_MODEL: qwen3.5:4b
      TRANSLATOR: qwen            # 一模兩用
      TARGET_LANG: zho_Hant
    volumes: ["./storage:/app/storage"]   # 上傳、報告、SQLite 快取
    depends_on: [summarize-service]
    ports: ["8001:8000"]

  frontend:                       # Vite 靜態檔 → nginx 反代 API 到 gateway（同源）
    build: ./frontend
    depends_on: [gateway]
    ports: ["8080:80"]
```
...
| 方式 | 說明 | 取捨 |
|---|---|---|
| ① 烤進 image（採用）| build 期 `ollama pull` 進映像層 | 自帶模型、零外部下載；image ~3GB |
| ② volume 掛載 | 權重放 volume，image 不含 | image 小、可熱換；部署要另備權重 |
| ③ 首次開機拉取 | entrypoint 首次 `ollama pull` | image 小、自動化；首次需連網 |

- **MLX 版不可用**（Apple Silicon 專屬，Linux 雲端跑不動）→ 雲端一律 GGUF。
- 記憶體：Qwen3.5-4B Q4（~4GB KV 含）+ gateway（~0.8GB）+ OS ≈ **~6GB，8GB 可跑**（低並發）。
...
| **7（可選）優化** | 分段摘要去重、GPU 自動升級（掃描版 OCR fallback 已於 gateway 內建 RapidOCR） |
...
- 掃描版（圖片型）PDF：**已內建 RapidOCR fallback**（某頁文字量過少時自動 render 成圖 OCR，純 CPU、免系統套件）；`DISABLE_OCR=1` 可關。高精度需求（複雜表格/公式）可另接 DeepSeek-OCR 作 GPU 選配層。
