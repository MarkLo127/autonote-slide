# main.py — Streamlit 前端（整合後端 + 模型選擇 + 主題）
...
def call_process_one_file(uploaded_file, theme_preset=None, footer_text=None, llm_baseurl=None, llm_apikey=None, llm_model=None):
    """
    呼叫後端 /api/process
    ...
    if llm_baseurl:
        data["llm_baseurl"] = llm_baseurl
    if llm_apikey:
        data["llm_apikey"] = llm_apikey
    if llm_model:
...
# 主題設定（簡報/心智圖頁面會用到）
with st.sidebar.expander("🎨 簡報主題設定", expanded=(page in ["心智圖生成", "簡報生成"])):
...
def section_summary():
    ...
    run = st.button("開始摘要", type="primary", disabled=(file is None), use_container_width=True)
    ...
    if run and file:
        with st.status("讀取並處理中…", expanded=True) as status:
            try:
                ...
                st.download_button("下載 PDF", data=pdf_bytes, file_name="converted.pdf", mime="application/pdf", use_container_width=True)
...
def section_keywords():
    ...
    file = st.file_uploader("上傳單一文件", type=["pdf", "docx", "doc", "pptx", "ppt", "md", "txt", "jpg", "png"], accept_multiple_files=False)
    run = st.button("開始擷取", type="primary", disabled=(file is None), use_container_width=True)
...
def section_mindmap():
    ...
    run = st.button("生成心智圖（.mmd 下載）", type="primary", disabled=(file is None), use_container_width=True)
    ...
    if run and file:
        with st.status("讀取並處理中…", expanded=True) as status:
            try:
                ...
                st.download_button("下載 mindmap.mmd", data=mmd.encode("utf-8"), file_name="mindmap.mmd", mime="text/plain", use_container_width=True)
...
def section_slides():
    ...
    file = st.file_uploader("上傳單一文件", type=["pdf", "docx", "doc", "pptx", "ppt", "md", "txt", "jpg", "png"], accept_multiple_files=False)
    run = st.button("開始生成簡報", type="primary", disabled=(file is None), use_container_width=True)
